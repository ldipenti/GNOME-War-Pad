# Starchart widget prototype
# Based on a contribution from Nicolas D. Cesar, thanks Nico! :-)
...
class Starchart(gtk.DrawingArea):
    ...
    def __init__(self, planets, ships):
        ...
        self.planets = map(lambda x: StarchartFloatingObject(x),
                           planets.values())
        self.ships = map(lambda x: StarchartFloatingObject(x), ships.values())
        ...
        self.names = map(lambda p: self.create_pango_layout(p.name), self.planets)
    ...
    def on_button_press_event(self, w,e):
        ...
        pid = find_closest_planet(e.x, e.y, self.planets)
        print pid
        self.planets[pid].selected = not self.planets[pid].selected
        self.draw()
        # Redraw widget
        self.queue_draw()
    ...
    def _draw_a_planet(self, cr, p, radius):
        ...
        if p.selected:
            ...
            cr.rectangle (my_x - (radius + 2), my_y - (radius + 2),
...
def find_closest_planet(x, y, planets):
    closest=0
    dist=999999999999
    for a in planets:
