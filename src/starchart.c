...
void draw_ship (gpointer key, gpointer value, gpointer user_data)
{
    ...
    if (ship_valid_coords(ship)) {
      vp_coord_v2w (ship_get_x_position(ship), ship_get_y_position(ship), &xi, &yi);
...
void draw_planet (gpointer key, gpointer value, gpointer user_data)
{
  ...
  if (planet_valid_coords(planet)) {
    ...
    //gtk_object_set_data (GTK_OBJECT (item), "planet_data", planet);
...
//    GnomeCanvasGroup *grid_group;
	...
	starchart_set_grp_grid(GNOME_CANVAS_GROUP (gnome_canvas_item_new 
						(starchart_get_grp_root(), GNOME_TYPE_CANVAS_GROUP, NULL)));
    ...
    background = gnome_canvas_item_new (starchart_get_grp_root(), GNOME_TYPE_CANVAS_RECT,
				      "outline_color", "grey",
    ...
    // Sets starchart grid
    //grid_group = GNOME_CANVAS_GROUP (gnome_canvas_item_new
	//		(starchart_get_grp_root(), GNOME_TYPE_CANVAS_GROUP, NULL));
    ...
    for (i = 0; i < (STARCHART_WIDTH_INT / QUAD_WIDTH_INT) + 1 ; i++)
    {
      ...
      grid_line = gnome_canvas_item_new (starchart_get_grp_grid(), GNOME_TYPE_CANVAS_LINE,
    ...
    // Various bindings
    /*
    gtk_object_set_data (GTK_OBJECT (starchart_get_canvas()), 
...
void
starchart_scroll (gint scroll_x, gint scroll_y)
{
  gint x, y;
...
GSList *
starchart_get_surrounding_quads (GSList * objects_per_quad[TOTAL_QUADS],
				 gint center_quad)
{
  GSList *objects = NULL;
...
GnomeCanvasItem *
starchart_find_nearest_object (GSList * objects_in_quad, gdouble x, gdouble y)
{
  gint nr, i;
...
void
starchart_unhighlight_planet (GnomeCanvasItem * planet)
{
  gdouble x, y;
...
void
starchart_unhighlight_ship (GnomeCanvasItem * ship)
{
  gdouble x, y;
...
GnomeCanvasItem *
starchart_highlight_nearest_planet (GSList * planets_in_quad, gdouble wx,
				    gdouble wy)
{
...
GnomeCanvasItem *
starchart_highlight_nearest_ship (GSList * ships_in_quad, gdouble wx,
				  gdouble wy)
{
...
gboolean
starchart_is_my_planet (GnomeCanvasItem * planet_item)
{
  gdouble x, y;
  Planet *planet = NULL;
...
gboolean
starchart_is_my_ship (GnomeCanvasItem * ship_item)
{
...
void
starchart_select_nearest_planet (GtkWidget * gwp, GSList * planets_nearby,
...
void
starchart_select_nearest_ship (GtkWidget * gwp, GSList * ships_nearby,
			       gdouble wx, gdouble wy)
{
...
// (x, y) in world coords
gint
get_quadrant (gdouble x, gdouble y)
{
  gint xi, yi;
  xi = (gint) x;
  yi = (gint) y;
...
void
starchart_get_object_center_coord (GnomeCanvasItem * item, gdouble * x,
				   gdouble * y)
{
  gdouble x1, y1, x2, y2;
...
void
starchart_zoom_in (GnomeCanvas * starchart)
{
  gdouble zoom;
...
void
starchart_zoom_out (GnomeCanvas * starchart)
{
  gdouble zoom;
  zoom = game_get_starchart_zoom ();
